...
class MongoData(object):
    ...
    def get_table_data(self, start_date=None, end_date=None, product_search=None, active_search=None, inactive_search=None,company_search = None):
        ...
        if product_search:
            ...
            result.extend(self.sanitize_list(company_search))
            search_query.update({
                'company': {'$in': result}
            })
    ...
    def get_search_bar_data(self):
        ...
        company_names = set()
        # for i in self.lyophilized_db_obj.get_records():
        #     company_names.update(set(records.get('company', [])))
        for records in self.lyophilized_db_obj.get_records():
            ...
            company_names.add(records.get('company'))

        return (list(products), list(active_ingredients), list(inactive_ingredients), list(company_names))
            ...
            for record in self.lyophilized_db_obj.get_records():
                ...
                for product in products:
                    ...
                    for company_name in company:
                        # print(company_name)
                        id = product + company_name #???
                        if id not in products_set:
                            products_set.add(id)
                            existing_val = company_dict.get(company_name, {}).get(date_value, 0)
                            total_val = company_dict.get(company_name, {}).get('total_count', 0)
                            if company_dict.get(company_name):
                                company_dict[company_name].update({date_value: existing_val + 1})
                                company_dict[company_name].update({'total_count': total_val + 1})
                            else:
                                company_dict[company_name] = {}
                                company_dict[company_name].update({date_value: 1})
                                company_dict[company_name].update({'total_count': 1})

        except Exception as exc:
            log.do_error(f"Exception occurred while fetching ingredients from Database, error: {str(exc)}")

        return {'active': active_ingredients_dict, 'inactive': inactive_ingredients_dict, 'company': company_dict}


    def get_timeseries_dataframe(self):

        time_series_data = self.generate_occurences_data()
        # for v in company_series:
        #     print(v)
        inactive_list = list()
        ...
        company_list = list()
        company_ing_list = list()
        company_dates_list = list()
        company_occ_list = list()
        company_total_count = list()
        ...
        for company, value in time_series_data.get('company', {}).items():
            # print(value)
            company_ing_list.append(company)
            for key, count in value.items():
                if key == "total_count":
                    company_total_count.append(count)
                else:
                    company_dates_list.append(int(key))
                    company_list.append(ingredient)
                    company_occ_list.append(count)
        ...
        company_series = pd.DataFrame({
            ...
            "Year": company_dates_list,
            "Occurences": company_occ_list
        })
        ...
        company_chart = pd.DataFrame({
            "Company": company_ing_list,
            "Occurences": company_total_count
        })
        active_time_series = active_time_series.sort_values(by=['Year'])
        inactive_time_series = inactive_time_series.sort_values(by=['Year'])
        company_series = company_series.sort_values(by=['Year'])
        active_chart = active_chart.sort_values(by=['Occurences'])
        inactive_chart = inactive_chart.sort_values(by=['Occurences'])
        company_chart = company_chart.sort_values(by=['Occurences'])
        return (active_chart, inactive_chart, company_chart,active_time_series, inactive_time_series,company_series)