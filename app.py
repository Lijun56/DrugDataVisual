    ...
    [
        ...
        dbc.Nav(
            [
                ...
                dbc.NavLink("Display Comp-Time-Series Data", href="/comp_time", active="exact"),
            ],
            vertical=True,
            pills=True,
        ),
...
ACTIVE_CHART, INACTIVE_CHART, COMPANY_CHART, ACTIVE_TIMESERIES, INACTIVE_TIMESERIES, COMPANY_TIMESERIES = UI_DATA_OBJ.get_timeseries_dataframe()
# PRODUCTS_LIST, ACTIVE_INGREDIENTS_LIST, INACTIVE_INGREDIENTS_LIST = UI_DATA_OBJ.get_search_bar_data()
PRODUCTS_LIST, ACTIVE_INGREDIENTS_LIST, INACTIVE_INGREDIENTS_LIST,COMPANY_LIST = UI_DATA_OBJ.get_search_bar_data()
# for i in COMPANY_TIMESERIES:
# for k, v in ACTIVE_TIMESERIES.items():
# for k, v in COMPANY_TIMESERIES.items():
#     print(v)
...
def render_page_content(pathname):
    ...
    elif pathname == '/comp_time':
        return comp_generate_time_series()
...
def get_selection_options(table_option):
    ...
    # elif table_option == "company_dropdown":
    #     return [{'label': i, 'value': i} for i in sorted(COMPANY_LIST)]
    else:
        raise PreventUpdate
...
def display_table(selection_dropdown, table_dropdown):
    ...
    #print(product_dropdown, active_dropdown, inactive_dropdown)
...
def display_timeseries_table(inactive_time_series_dropdown):
    ...
    print(inactive_time_series_dropdown)
...
'''
*************************************************
----- COMPANY TIME SERIES PAGE -----
*************************************************
'''


def comp_get_era_marks():
    era_marks = {}
    for i in range(1954, 2023, 4):
        era_marks.update({i: {'label': str(i)}})
...
def comp_generate_time_series():
    return  html.Div([
                    dcc.Dropdown(
                        id='company_time_series_dropdown',
                        options=[{'label': i, 'value': i} for i in sorted(COMPANY_LIST)],
...
@app.callback(
    Output("timeseries_comp_table", "children"),
    [ 
        Input("company_time_series_dropdown", "value")
    ]

)
def comp_display_timeseries_table(company_time_series_dropdown):

    if not company_time_series_dropdown:
        raise PreventUpdate

    print(company_time_series_dropdown)
    records_rows = UI_DATA_OBJ.get_table_data(company_search=company_time_series_dropdown)
...
@app.callback(
    Output("time_comp_chart", "figure"),
    [Input("company_time_series_dropdown", "value"),
    Input("era-slider", "value")]
)
def comp_display_timeseries_graph(company_time_series_dropdown, year_range):
    ...
    company_df = None
    print(year_range)
    if company_time_series_dropdown:
        company_df = COMPANY_TIMESERIES[COMPANY_TIMESERIES['Company'].isin(company_time_series_dropdown)]
        
        if year_range:
            company_df = company_df[(company_df.Year >= year_range[0] )&( company_df.Year <= year_range[1] )]
        else:
            company_df = company_df[( company_df.Year >= 1954 )&( company_df.Year <= 2022)]
     
    fig = px.line(company_df, x='Year', y='Occurences', color='Company')
    fig.update_traces(mode='markers+lines')
    return fig
